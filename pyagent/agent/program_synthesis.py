...
class ProgramSynthesizer:
    ...
    def synthesize_from_examples(self, spec: SynthesisSpec) -> SynthesisResult:
        ...
        # Try each template.
        for _template_name, template_code in self.TEMPLATES.items():
    ...
    def _verify_against_examples(self, code: str, spec: SynthesisSpec) -> bool:
        ...
        try:
            ...
            if func is None:
                return False
            for inp, expected in zip(spec.inputs, spec.outputs, strict=False):
    ...
    def _infer_transformation(self, spec: SynthesisSpec) -> str:
        ...
        for inp, out in zip(spec.inputs, spec.outputs, strict=False):
            if isinstance(inp, (int, float)) and isinstance(out, (int, float)):
                ...
                diff = out - inp
                if all((o - i) == diff for i, o in zip(spec.inputs, spec.outputs, strict=False) if isinstance(i, (int, float)) and isinstance(o, (int, float))):
                    return f"def {spec.function_name}(x):\n    return x + {diff}\n"
