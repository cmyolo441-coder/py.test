            ...
            try:
                for line in resp.iter_lines():
                    ...
                    for tc in msg.get("tool_calls", []) or []:
                        fn = tc.get("function", {})
                        idx = tc.get("index", len(tool_fragments))
                        ...
                        if isinstance(args, dict):
                            # Already a dict (non-streaming chunk) — store as JSON string
                            # so later chunks can concatenate.
                            frag["arguments"] += json.dumps(args)
                        else:
                            frag["arguments"] += str(args)
