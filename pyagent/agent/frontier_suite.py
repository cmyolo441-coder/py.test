...
class Neuroevolution:
    ...
    def _crossover(self, a: Genome, b: Genome) -> Genome:
        ...
        child_weights = []
        for layer_a, layer_b in zip(a.weights, b.weights, strict=False):
            child_layer = []
            for row_a, row_b in zip(layer_a, layer_b, strict=False):
                child_row = [random.choice([wa, wb]) for wa, wb in zip(row_a, row_b, strict=False)]
...
class AdversarialDefense:
    ...
    def scan(self, user_input: str) -> dict[str, Any]:
        ...
        for _i, pattern in enumerate(self.injection_patterns):
