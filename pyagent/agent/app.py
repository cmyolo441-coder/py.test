...
class App:
    ...
    def _handle_turn(self, user_input: str) -> None:
        ...
        # Auto-orchestrate complex tasks with parallel sub-agents.
        analysis = should_auto_orchestrate(user_input, self.config)
    ...
    def _handle_orchestrated_turn(self, user_input: str, analysis) -> None:
        """Handle a complex task by auto-orchestrating parallel sub-agents.

        This runs multiple specialist sub-agents in parallel, merges their
        results, and feeds the merged output back into the conversation as
        if the main agent had produced it.
        """
        import time as _time

        from .multi_agent import MultiAgentOrchestrator

        # Inform the user about auto-orchestration.
        specialists_str = ", ".join(analysis.recommended_specialists)
        self.ui.info(
            f"\u26a1 Complex task detected — auto-orchestrating with "
            f"{len(analysis.recommended_specialists)} specialists ({specialists_str})"
        )
        ...
        if merged:
            # Feed the merged output as the assistant's response.
            self.conversation.add_user(user_input)
            self.conversation.add_assistant(merged)
            self.conversation.save()

            # Render the merged output with streaming.
            renderer = self.ui.stream_response()
            renderer.start_thinking()
            # Simulate streaming the merged output.
            chunk_size = 50
            for i in range(0, len(merged), chunk_size):
                renderer.on_delta(merged[i:i + chunk_size])
                _time.sleep(0.01)  # Small delay for smooth streaming effect
            renderer.finish(merged)
        else:
            self.ui.warn("No output from sub-agents — falling back to single agent.")
            self._handle_turn(user_input)

        # Defer post-turn bookkeeping.
        self._pending_post_turn = {
            "final": merged,
            "turn_start": _time.perf_counter(),
            "in_tokens": len(user_input.split()),
            "used_tool": False,
        }
