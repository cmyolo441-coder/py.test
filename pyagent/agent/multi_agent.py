...
class MultiAgentOrchestrator:
    ...
    def _run_specialist(self, specialist: str, task: str) -> SubAgentResult:
        ...
        Thread-safe: does NOT modify shared app.config state.  Passes the
        specialist system prompt directly to the provider in the messages list.
        """
        ...
        try:
            ...
            if provider is None:
                return SubAgentResult(specialist, task, "", time.perf_counter() - start, False,
                                      "agent not initialised — run /provider first")
            # Build a fresh one-shot conversation with the specialist prompt.
            messages = [
    ...
    def run_parallel(self, task: str, specialists: list[str]) -> OrchestrationResult:
        ...
        tracker = ProgressTracker(Console(), use_live=True)
        tracker.start(
            f"Running {len(specialists)} specialists in parallel",
            specialists,
        )

        # Use a reasonable per-specialist timeout (120s) instead of the
        # previous 700 000s wall-clock timeout which could hang forever.
        per_spec_timeout = 120

        # Mark all as running
        for s in specialists:
            tracker.task_start(s)
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=min(6, len(specialists))) as pool:
            ...
            for fut in concurrent.futures.as_completed(futures, timeout=per_spec_timeout * len(specialists)):
                s = futures[fut]
                try:
                    result = fut.result(timeout=per_spec_timeout)
                    results.append(result)
                    if result.success:
                        tracker.task_complete(s, result.output[:50])
                    else:
                        tracker.task_fail(s, result.error)
                except concurrent.futures.TimeoutError:
                    results.append(SubAgentResult(s, task, "", per_spec_timeout, False, "timeout"))
                    tracker.task_fail(s, "timeout")
                except Exception as exc:  # noqa: BLE001
                    results.append(SubAgentResult(s, task, "", 0.0, False, str(exc)))
                    tracker.task_fail(s, str(exc))
