...
class GoalMode:
    ...
    def _execute_turn(self, instruction: str, cancel_token: CancellationToken) -> str:
        ...
        from .context_manager import compress_messages
        self.app.conversation.messages = compress_messages(
            self.app.conversation.messages,
            self.app.config.resolved_model(),
            self.app.config.provider,
        )
